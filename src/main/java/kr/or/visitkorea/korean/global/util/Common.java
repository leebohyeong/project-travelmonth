...
public class Common {
	...
	public static ArrayList<JSONObject> getThemes()
	{
		return new ArrayList<JSONObject>(){{
			...
			add(new JSONObject(){{ put("theme", "c"); put("title", "친환경<br>여행"); put("sub_title", "여행트렌드 3"); put("main_title", "친환경여행"); put("content", "환경이슈에 대한 관심이 증가하는 추세에 맞추어 플로깅, <br>해변정화, 무동력 수단 이용 등 작지만 의미있는 탄소줄이기를 실천해"); }});
			...
			add(new JSONObject(){{ put("theme", "e"); put("title", "열린<br>관광"); put("sub_title", "여행트렌드 5"); put("main_title", "열린관광"); put("content", "누구나 누릴 수 있는 모두를 위한 장애물 없는 관광 “열린관광”, <br>한국관광공사가 추천하는 열린관광지를 따라 편안하고 안전한 여행을 하세요"); }});
			add(new JSONObject(){{ put("theme", "f"); put("title", "K-<br>CULTURE"); put("sub_title", "여행트렌드 6"); put("main_title", "K-Culture"); put("content", "케이팝, 드라마를 넘어 이제는 예능, 웹툰,뷰티, 패션까지. K-Culture는 계속해서 <br>확장하고 있습니다. K-Culture 200% 즐기러 떠나보세요!"); }});
			add(new JSONObject(){{ put("theme", "g"); put("title", "스포츠<br>케이션"); put("sub_title", "여행트렌드 7"); put("main_title", "스포츠케이션"); put("content", "걷기, 등산 등 야외활동과 서핑, 열기구 등 국내 다양한 아웃도어 <br>레저여행을 알려드립니다."); }});
