<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ include file="../include/include-taglib.jspf" %>
<!DOCTYPE html>
<html lang="ko">
<head>
    <%@ include file="../include/include-head.jspf" %>
</head>
<body id="travelmonth-event-june">
<%@ include file="../include/include-header.jspf" %>
<div id="container">
    <article class="event-june">
        <section>
            <div class="screen_out"><h2>여행가는 JUNE <br>
                같이 가 ~ 준!</h2>
                <p>나랑 여행 갈래요? 같이 여행가기로 약속!</p></div>
        </section>
        <section>
            <div class="screen_out"><h2>6월 여행셀럽 준과 떠나는 <br>
                특별한 여행 찬스</h2>
                <p>지금 바로 사연과 함께 신청해 주세요!</p>
                <p><strong>모집인원</strong> 회차별 20명</p>
                <p><strong>이벤트 기간</strong> 2023.5.17(수) ~ 5.21(일)</p>
                <p><strong>당첨자 발표</strong> 개별 연락을 통해 별도 안내 예정</p></div>
        </section>
        <section>
            <div class="screen_out"><h2>준이와 함께 떠나는 1차여행</h2>
                <p>5/25(목) 대전 출발 → 전북 고창 도착(왕복)</p>
                <p>#농촌관광 #고창농촌팜파티 #당일치기</p>
                <p>농촌으로 떠나는 작은 여행</p>
                <p>고창농촌팜파티</p>
                <p>주요 일정 안내</p>
                <button type="button"
                        data-gtag-action="2023 여행가는 달_같이가 준 이벤트"
                        data-gtag-category="june_time_table_1"
                        data-ga-category="2023 여행가는 달_같이가 준 이벤트"
                        data-ga-action="타임테이블1 확인하기">타임테이블 확인하기
                </button>
                <p>대전 출발 → 선운사 및 녹차밭 관람 → <br>
                    농가체험장 쉼드림(중식) → 핸드드립 맷돌 커피& <br>
                    복분자주스 → 편백숲 치유프로그램→ <br>
                    복분자 족욕 체험 → 대전 도착 → 해산</p>
                <ul>
                    <li>선운사 및 녹차밭</li>
                    <li>고창농가체험(쉼드림)</li>
                </ul>
                <h2>준이와 함께 떠나는 2차여행<p>5/29(월) 서울 출발 → 강원도 춘천 도착(왕복)</p>
                    <p>#힐링/웰니스 #춘천마임축제2023 #당일치기</p>
                    <p>호반의 도시 춘천의 아름다운 조망과 <br>
                        세계적인 축제 마임축제와 함께 하는 여행</p>
                    <p>고춘천마임축제</p>
                    <p>주요 일정 안내</p>
                    <button type="button">타임테이블 확인하기</button>
                    <p>서울 출발 → 삼악산케이블카 탑승 → 중식 → <br>
                        티타임 → 의암호 스카이워크 → 마임공연 관람 → <br>
                        서울 도착 → 해산</p>
                    <ul>
                        <li>삼악산케이블카</li>
                        <li>의암호 스카이워크</li>
                        <li>마임축제</li>
                    </ul>
                    <p>※ 해당 단체여행 프로그램의 경우,
                        &#60;김해준&#62; 유튜브 채널에 콘텐츠로 사용될 수 있습니다
                    </p>
                </h2>
            </div>
            <button type="button"
                    data-bs-toggle="modal"
                    data-bs-target="#juneModal1"
                    data-gtag-action="2023 여행가는 달_같이가 준 이벤트"
                    data-gtag-category="june_time_table_1"
                    data-ga-category="2023 여행가는 달_같이가 준 이벤트"
                    data-ga-action="타임테이블1 확인하기">
                <span class="screen_out">타임테이블 확인하기</span>
            </button>
            <button type="button"
                    data-bs-toggle="modal"
                    data-bs-target="#juneModal2"
                    data-gtag-action="2023 여행가는 달_같이가 준 이벤트"
                    data-gtag-category="june_time_table_2"
                    data-ga-category="2023 여행가는 달_같이가 준 이벤트"
                    data-ga-action="타임테이블2 확인하기">
                <span class="screen_out">타임테이블 확인하기</span>
            </button>
        </section>
        <section>
            <div class="screen_out"><h2>참여방법</h2>
                <ul>
                    <li><strong>STEP 1</strong> 아래의 [준이랑 여행가기] 버튼 클릭</li>
                    <li><strong>STEP 2</strong> <br>
                        구글 시트에서 희망하는 단체여행을 선택하고 <br>
                        준이와 여행을 떠나고 싶은 사연 입력
                    </li>
                    <li><strong>STEP 3</strong> <br>
                        구글 시트에서 신청인 개인정보와 <br>
                        동반인 이름(선택사항, 최대 1인), <br>
                        본인과의 관계를 입력
                    </li>
                </ul>
                <a href="https://docs.google.com/forms/d/e/1FAIpQLSetdfsCSIw-XkGcmGRyMy8jGmI5YC43XHXH2ZZYfmlRxbmzkg/viewform?vc=0&c=0&w=1&flr=0&fbzx=-7716034740995790776"
                   target="_blank"
                   data-gtag-action="2023 여행가는 달_같이가 준 이벤트"
                   data-gtag-category="june_with_go"
                   data-ga-category="2023 여행가는 달_같이가 준 이벤트"
                   data-ga-action="준이랑 여행가기">준이랑 여행가기</a></div>
            <a href="https://docs.google.com/forms/d/e/1FAIpQLSetdfsCSIw-XkGcmGRyMy8jGmI5YC43XHXH2ZZYfmlRxbmzkg/viewform?vc=0&c=0&w=1&flr=0&fbzx=-7716034740995790776"
               target="_blank"
               data-gtag-action="2023 여행가는 달_같이가 준 이벤트"
               data-gtag-category="june_with_go"
               data-ga-category="2023 여행가는 달_같이가 준 이벤트"
               data-ga-action="준이랑 여행가기"><span class="screen_out">준이랑 여행가기</span></a></section>
        <section>
            <div class="screen_out">
                <h2>유의사항</h2>
                <p>이벤트는 개인정보 연락처 기준 1인 당 1회만 참여 가능합니다. (중복참여 불가)</p>
                <p>단체여행 당첨자에게는 개별 전화 혹은 문자를 통해 단체여행 당첨 및 프로그램 관련 안내를 드립니다.</p>
                <p>참가신청 마감 후에는 여행 참가자의 변경과 양도가 불가능합니다.</p>
                <p>여행 신청 대표자 및 동반자의 잘못된 개인정보 입력으로 인한 당첨의 불이익은 책임지지 않습니다.</p>
                <p>단체여행은 출발지에서 버스에 탑승하여 이동하며, 집결장소와 출발/도착시간과 관련 자세한 사항은
                    별도 개별 연락을 통해 안내해 드립니다.</p>
                <p>
                    단체 여행은 영상으로 촬영될 예정이며, 영상 콘텐츠는 &lt;한국관광공사&gt;,
                    &lt;김해준&gt; 채널에 컨텐츠에 사용될 수 있습니다. 초상권 동의하신 분에 한에 여행 참여가 가능합니다.
                </p>
                <p>
                    본 단체여행 이벤트에는 여행자 보험이 포함되어 있으며, 여행에 이용되는 교통수단(전세버스)은
                    별도의 손해배상 보험에 가입되어 있습니다.</p>
                <p>
                    개인정보동의로 수집된 개인정보는 개인정보 보호법 규정에 의거하여 처리되며,
                    이벤트 종료 후 3개월간 보유되며, 이 후에는 안전한 방식으로 폐기됩니다.
                </p>
            </div>
        </section>
        <div class="event-june__modal modal fade" id="juneModal1" data-bs-backdrop="static" data-bs-keyboard="false"
             tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"><h2 class="modal-title screen_out">고창농촌팜파티</h2>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"><span
                                class="screen_out">닫기</span></button>
                    </div>
                    <div class="modal-body"><img src="/travelmonth/assets/images/event/june/img_event_june_modal_1.png">
                        <div class="screen_out">
                            <table>
                                <thead>
                                <tr>
                                    <th>일정</th>
                                    <th>시간</th>
                                    <th>내용</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr>
                                    <td>집결(대전역)</td>
                                    <td>8:00</td>
                                    <td>대전역 주차장앞 출발→ 경부선 고속철도1번 출구 주차장</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>9:00-10:50</td>
                                    <td>고창 선운사 이동</td>
                                </tr>
                                <tr>
                                    <td>선운사</td>
                                    <td>11:00~12:30</td>
                                    <td>선운사 및 녹차밭 관람</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>12:30~12:40</td>
                                    <td>농가체험장 쉼드림 도착 및 프로그램 소개</td>
                                </tr>
                                <tr>
                                    <td>점심식사</td>
                                    <td>12:40~13:30</td>
                                    <td>중식(바비큐세팅 및 팜파티)</td>
                                </tr>
                                <tr>
                                    <td>간단한 티타임</td>
                                    <td>13:30~14:00</td>
                                    <td>핸드드립 맷돌커피 체험 & 복분자주스 시음</td>
                                </tr>
                                <tr>
                                    <td rowspan="2">편백수 치유프로그램</td>
                                    <td>14:00~15:00</td>
                                    <td>편백숲 치유프로그램
                                        자연과 하나되는 피크닉~
                                        → 그룹미션
                                        (자연물로 표현하기)
                                        → 보물찾기
                                        (꽃화분으로 교환)
                                        → 사진찍기 등등
                                        → 고무신 + 밀짚모자 +
                                        요가매트 + 개떡 간식체험
                                    </td>
                                </tr>
                                <tr>
                                    <td>15:00~16:00</td>
                                    <td>복분자 족욕 체험</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>16:00~18:30</td>
                                    <td>대전 도착 및 해산</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="event-june__modal modal fade" id="juneModal2" data-bs-backdrop="static" data-bs-keyboard="false"
             tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header"><h2 class="modal-title screen_out">춘천마임축제</h2>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"><span
                                class="screen_out">닫기</span></button>
                    </div>
                    <div class="modal-body"><img src="/travelmonth/assets/images/event/june/img_event_june_modal_2.png">
                        <div class="screen_out">
                            <table>
                                <thead>
                                <tr>
                                    <th>일정</th>
                                    <th>시간</th>
                                    <th>내용</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr>
                                    <td>집결 (동서울버스터미널)</td>
                                    <td>8:00</td>
                                    <td>동서울터미널 집결</td>
                                </tr>
                                <tr>
                                    <td>서울 → 춘천 <br>
                                        (삼악산케이블카 주차장)
                                    </td>
                                    <td>08:00~9:30</td>
                                    <td>삼악산케이블카 주차장 도착</td>
                                </tr>
                                <tr>
                                    <td>삼악산케이블카 탑승</td>
                                    <td>10:00~11:00</td>
                                    <td>케이블카 탑승 및 관람</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>11:20~11:30</td>
                                    <td>식사장소 이동</td>
                                </tr>
                                <tr>
                                    <td>점심식사</td>
                                    <td>11:30~13:00</td>
                                    <td>한가족숯불닭갈비</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>13:10~13:20</td>
                                    <td>카페 이동</td>
                                </tr>
                                <tr>
                                    <td>간단한 티타임</td>
                                    <td>13:20~13:50</td>
                                    <td>카페 2월의 화실</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>13:50~14:00</td>
                                    <td>의암호 스카이워크로 이동</td>
                                </tr>
                                <tr>
                                    <td>의암호 스카이워크</td>
                                    <td>14:00~15:00</td>
                                    <td>의암호 스카이워크 산책</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>15:10~15:30</td>
                                    <td>마임공연장으로 이동</td>
                                </tr>
                                <tr>
                                    <td>마임공연관람</td>
                                    <td>15:30~17:30</td>
                                    <td>마임공연 관람</td>
                                </tr>
                                <tr>
                                    <td>버스탑승 및 이동</td>
                                    <td>17:30~19:00</td>
                                    <td>서울 도착 및 해산</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </article>
</div>
<%@ include file="../include/include-footer.jspf" %>
</body>
</html>